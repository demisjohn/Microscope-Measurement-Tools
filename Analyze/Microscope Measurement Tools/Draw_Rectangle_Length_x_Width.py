...
def run():
    ...
    topLine=(x[1],y[1],x[2],y[2])  
    bottomLine=(x[3],y[3],x[0],y[0]) 
    calLength=((calX[0]-calX[1])**2+(calY[0]-calY[1])**2)**0.5 # length in physical units
    rightLine=(x[0],y[0],x[1],y[1])
    leftLine=(x[2],y[2],x[3],y[3])
    calWidth=((calX[1]-calX[2])**2+(calY[1]-calY[2])**2)**0.5 # width in physical units

    # Determine the pixel coordinates to place the text label, in the center of the center-line
    textPointLength = ((leftLine[0]+leftLine[2])*0.5,(leftLine[1]+leftLine[3])*0.5,)
    textPointWidth = ((topLine[0]+topLine[2])*0.5,(topLine[1]+topLine[3])*0.5,)
    
    # format of measurement text:
    lenstr = "%0.1f" % calLength + " %s" % (unit)  # string to print
    widstr = "%0.1f" % calWidth + " %s" % (unit)  # string to print

    drawLines( [topLine, leftLine, bottomLine, rightLine] )
    drawText( lenstr, textPointLength, align='c' )
    drawText( widstr, textPointWidth, align='c' )
...
def drawText( text, xy, align='c' ):
    ...
    if align == 'rm':
        xtext, ytext = xy[0] - ( strw * 0.5) , xy[1] + ( strh * 0.5 )
    elif align == 'cb':

...
Draws the short dimension of a rectangle or rotated rectangle
Draws extension lines defined by long side width of the rectangle
'''
...
def run():
    ...
    # Test which side is the 'short' side to draw
    if l0 > l1:
        # Define the lines to draw as tuples: (x,y,x',y')
        topLine=(x[0],y[0],x[1],y[1]) # first 'long' side
        bottomLine=(x[2],y[2],x[3],y[3]) # the other 'long' side
        centerLine=((x[0]+x[1])*0.5,(y[0]+y[1])*0.5,(x[2]+x[3])*0.5,(y[2]+y[3])*0.5) # Connecting the 'long' sides through the middle, parallel to the short side
        calLength=((calX[1]-calX[2])**2+(calY[1]-calY[2])**2)**0.5 # length of the 'short' side in physical units
    else: 
        topLine=(x[1],y[1],x[2],y[2]) 
        bottomLine=(x[3],y[3],x[0],y[0]) 
        centerLine=((x[1]+x[2])*0.5,(y[1]+y[2])*0.5,(x[3]+x[0])*0.5,(y[3]+y[0])*0.5) 
        calLength=((calX[0]-calX[1])**2+(calY[0]-calY[1])**2)**0.5 
